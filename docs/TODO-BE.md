...
- [x] **Gravity Pulse (G):** only firable when the player is holding the wildcard charge. Apply a radial impulse to all planets, rockets, and caches within radius (falloff by distance), emit `event:'wildcardUse'` on successful activation, then consume the held charge.
