...
- [ ] BE sim loop runs at 60 Hz; snapshot scheduler emits `deltaSnapshot` at 60 Hz per connection; FE acks each applied snapshot with `ackSnapshot{tick}` (see `TODO-BE.md` sim loop + snapshots).
