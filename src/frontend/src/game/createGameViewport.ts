  ...
  const startViewport = async () => {
    ...
    try {
      ...
      const syncForesightVisualsToState = (
        state: Pick<typeof simulationState.currentState, "planets" | "suns">,
      ) => {
        ...
        renderFrame: (timeMs = performance.now()) => {
          ...
          if (
            ...
            const profilerSnapshot = profilingEnabled
              ...
              : null;
            const killFeed = simulationState.killFeedEntries.map((entry) => ({
              accent: entry.accent,
            ...
            emitHudState(
              buildLocalSandboxHudState({
                ...
                killFeed,
                planetAuraGap:
                  playerPlanetVisuals?.auraGap ?? lastHudState.planetAuraGap,
                planetAuraScale:
